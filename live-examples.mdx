...
These names were claimed by test wallets separate from the Orange namespace owner. Each username claim set its payment destination on chain without an owner signature. The [Orange example manifest](https://nova-claim-app.throbbing-cloud-0f8e.workers.dev/examples/orange.json) lists complete values and whether reverse and Primary selections are configured. Forward resolution alone does not create those selections.
