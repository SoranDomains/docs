...
The [claim app](https://nova-claim-app.throbbing-cloud-0f8e.workers.dev/) lets you choose `.nova` or `.orange` beside the username field. The same username is a different name in each namespace.

| Orange name | Destination | Memo or routing ID |
| --- | --- | --- |
| `cyclops.orange` | The same hello-world C contract as `cyclops.nova` | No memo; `hello()` returns `Hello, world!` |
| `nikcle.orange` | Classic G account | No memo |
| `mux.orange` | Muxed M address | Embedded routing ID `42`; no separate memo |
| `robert.orange` | Classic G account | Memo ID `77` |

These names were claimed by test wallets separate from the Orange namespace owner. Each username claim set its payment destination on chain without an owner signature. The [Orange example manifest](https://nova-claim-app.throbbing-cloud-0f8e.workers.dev/examples/orange.json) lists complete values and whether reverse and Primary selections are configured. As verified on 11 September 2026, `nikcle.orange`, `robert.orange` and `mux.orange` each have a namespace reverse name and a Primary name. `cyclops.orange` demonstrates forward contract resolution only. Forward resolution alone does not create display-name selections.
...
```sh
npm install @sorandomains/lookup@0.10.2 @stellar/stellar-sdk@17.0.1
```
