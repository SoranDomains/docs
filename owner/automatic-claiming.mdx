...
This guide uses **Owner 0.8.0** and **Holder 0.7.0** with the [native-claim testnet deployment](/reference/release-status). Older contracts need compatible native-claim support; updating a frontend alone does not add it.
...
Owner 0.8.0 and Holder 0.7.0 default to a **5 XLM network-fee limit** per
...
```bash
npm install @sorandomains/owner@0.8.0 @sorandomains/holder@0.7.0 @stellar/stellar-sdk@^17
```
...
Holder 0.7.0 checks Registrar attestation and deployed code after each receipt read or transaction confirmation. It requires RPC ledger context at least as recent as the result, including for a `null` receipt.
