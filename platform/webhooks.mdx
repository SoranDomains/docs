---
title: "Webhooks"
description: "Signed HTTP notifications for supported namespace events, dormancy signals and optional hosted services."
---
...
If the secret leaks or you never copied it, rotate it from the Developers tab. Rotation creates a fresh secret, shown once, for subsequent signatures. Deliveries already signed with the previous secret can still arrive; handle those according to your rotation policy and deduplicate on delivery ID. Rotation remains available to an authorized owner/admin even when billing would otherwise gate console extras.
