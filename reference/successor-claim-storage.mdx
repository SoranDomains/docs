...
Match the code hashes and contract addresses in the [current deployment](/reference/release-status). A version getter alone does not establish storage compatibility. The current Registry's immutable template pair does not migrate earlier Registry records.
