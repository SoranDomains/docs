...
Stores who owns a namespace and which contracts serve it. Registry code supports governance-authorized upgrades on the current testnet.
...
Policy terms are either `0` or `86400`–`3153600000` seconds. `trade_fee_bps` is `0`–`10000`; it is a basis-point field, not proof that a marketplace feature is enabled. The governed testnet rejects permanent code locking. Historical deployments retain their one-way permanence rules.
...
`set_primary` extends the record and instance/code. `clear_primary` removes the current entry, not its historical transactions. `touch_primary(address)` extends the entry if present and maintains instance/code. The governed Primary supports `upgrade(expected_hash, new_hash)` under Registry governance. The one-time migration interface verifies source elections before sealing.
...
Election and clear calls require the base G account's authorization. They maintain the instance without renewing shared code. Clear removes only the exact key and leaves transaction history. `touch_reverse_muxed(namespace,account,id)` and `touch_primary_muxed(account,id)` are permissionless: each maintains one existing entry and Lookup's instance, plus bounded code renewal as described below, without creating an entry or validating its proof. `keep_alive()` maintains the instance and performs bounded code renewal; it does not maintain every election. Archived keys require restoration; an unavailable read is not an empty election.
...
Registry, Allocator, Registrar and Resolver use these dynamic extension parameters. Primary retains the same parameters for its records and instance:
...
Lookup uses `threshold = min(2_500_000, max_ttl)` and `extend_to = min(3_000_000, max_ttl)` for its records and instance. These are ledger targets, not a promised number of days. A threshold-based bump only extends an entry below its threshold; it can be a no-op for an already-long TTL.

Code and instance TTLs are independent. The combined instance-extension operation checks both separately. [Instance TTL semantics](https://docs.rs/soroban-sdk/26.1.0/soroban_sdk/storage/struct.Instance.html#method.extend_ttl)

Lookup and Primary ordinary display-name writes use **instance-only extension**. They retain the existing record and instance targets without extending shared Wasm code. A newly uploaded replacement can therefore have a shorter code lifetime than the instance, without making the next display-name writer fund a long code renewal.

Explicit maintenance handles code separately: Lookup's `keep_alive`, `touch_reverse_muxed` and `touch_primary_muxed`, and Primary's `touch_primary`, extend code by **at most 17,280 additional ledgers per call**, capped at the configured target and network maximum. Repeated calls can accumulate extensions. The caller pays transaction costs and any storage rent; the ledger bound is not a fixed XLM price. Record and instance extension remains threshold-based and can cost more when those entries need renewal. This update does not change the other contracts' extension behavior or introduce sponsorship.
...
This Node example uses `@stellar/stellar-sdk@17.0.1` and `@sorandomains/lookup@0.10.2`. It reads a Registry namespace record and the Resolver's raw address, payment text and generation marker. It only reads state. Use an active name from the [live examples](/live-examples), or one you have issued.
...
## Governed testnet upgrade storage

Existing name and payment keys keep their encodings. Governed upgrades add separate keys:

| Contract | Key | Storage | Value |
| --- | --- | --- | --- |
| Registry | `UpgradeKey::AuthorityV1` | Instance | Governance `Address` |
| Registry | `UpgradeKey::ApprovedV1(role, hash)` | Persistent | `bool`; role `0` is Registrar, `1` is Resolver |
| Registry | `UpgradeKey::NativeV1(node)` | Persistent | `NativeCode` |
| Registrar / Resolver | `UpgradeModeKey::GovernedV1` | Instance | `bool` |

```rust
struct NativeCode {
    registrar: BytesN<32>,
    resolver: BytesN<32>,
    registrar_updates: u32,
    resolver_updates: u32,
}
```

Use `native_code(node)` for an existing namespace's expected executables. `template_hashes()` returns defaults for new factories. Primary's upgrade authority follows its configured Registry. Approved code changes retain addresses; replacing a contract creates separate storage.

The [migration storage reference](/reference/migration-storage) documents the additive lineage, import markers and election records across all six contracts.


### Bulk identity read helpers

The optimized Lookup uses these read-only helpers internally. They add no storage keys. Callers should use Lookup’s `reverse_names` and `primary_names` for verified display names: the raw snapshots below are not independently verified identity answers.

| Contract / method | Exact `scValToNative` result |
| --- | --- |
| Registrar `identity_read_version()` | `1` |
| Registrar `identity_context()` | `[registryAddress, namespaceNodeBytes, 1]` |
| Registrar `live_records(nodes)` | Array of `null` or `[holderAddress, generationBigInt, builtinAddress]`; at most 32 nodes |
| Resolver `identity_read_version()` | `1` |
| Resolver `identity_context()` | `[registryAddress, registrarAddress, 1]` |
| Resolver `reverse_records(addresses)` | Array of `null` or `[nameString, nodeBytes, generationBigInt]`; at most 32 addresses |
| Resolver `identity_destinations(requests)` | Array of `null` or `[paymentDestination, explicitForwardBoolean]`; at most 32 requests |
| Primary `identity_read_version()` | `1` |
| Primary `primary_records(addresses)` | Array of `null` or `[nameString, namespaceNodeBytes]`; at most 16 addresses |

Each destination request is `[nodeBytes, generationBigInt, builtinAddress]`. Its result uses the [existing destination enum](/reference/lookup-decoding). The supplied generation and fallback address are inputs, so the result alone is not proof of a live record. Lookup obtains them independently from the current Registrar and checks each election against current namespace routing. `explicitForwardBoolean` identifies an explicit forward address at that generation; G/C reverse verification requires it.

These optional tuple results encode absence as Soroban `Void`, decoded to JavaScript `null`. Lookup’s public `IdentityName::None` is a different enum and decodes to `["None"]`. Input counts above the raw helper limit reject the whole call (`BatchTooLarge`: Registrar `8`, Resolver `31`, Primary `13`).

The current optimized path can return related records and their contract context together:

| Contract / method | Exact `scValToNative` result | Maximum inputs |
| --- | --- | --- |
| Registry `native_contracts_many(nodes)` | Array of `null` or `[registrarAddress, resolverAddress]` | 16 nodes |
| Registrar `identity_live(nodes)` | `[registryAddress, namespaceNodeBytes, liveRecords]` | 32 nodes |
| Resolver `identity_reverse(addresses)` | `[registryAddress, registrarAddress, reverseRecords]` | 32 addresses |
| Resolver `identity_payments(requests)` | `[registryAddress, registrarAddress, destinations]` | 32 requests |
| Primary `identity_records(addresses)` | `[registryAddress, primaryRecords]` | 16 addresses |

`liveRecords`, `destinations` and `primaryRecords` use the corresponding row shapes above. Each `reverseRecords` row is `null` or `[nameString, nodeBytes, generationBigInt, paymentOrNull]`, where `paymentOrNull` is `null` or `[paymentDestination, explicitForwardBoolean]`. A missing payment does not turn an election into a verified result. Lookup checks the current Registrar generation, namespace pointers and full destination before returning a display name.

These helpers preserve input order and duplicates. Their outputs are raw observations for the verification process. Apps should use Universal Lookup's public methods for identity answers.
