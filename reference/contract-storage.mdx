...
Retained Rejected/Withdrawn claims are maintained alongside active claims, but `touch_claim` does not extend every claimant's separate `RejectedLock` or `ParkedAge` entry. Claimant history and events outside the indexer's known coverage need separate maintenance or, when archived, restoration. Archival does not erase these rules or establish a claim bypass. Confirm actual upkeep before relying on continued availability.
