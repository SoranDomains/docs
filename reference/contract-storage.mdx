...
This reference describes the native-claim deployment verified on **6 September 2026 at ledger 4,534,629**, plus the additive Lookup muxed identity upgrade verified on **7 September at ledger 4,549,326**. Match the contract's current Wasm hash to the [public deployment manifest](/reference/deployments/testnet.json) before decoding storage with this schema. An upgrade can change an upgradeable contract's implementation while keeping its address.
...
<Note>
The complete native claim types and request receipts are defined in the
[native claim storage reference](/reference/successor-claim-storage). The previous
5 September deployment has its own [legacy schema](/reference/legacy-contract-storage);
its Registry records do not contain the new required ownership epoch.
</Note>
...
| --- | --- | --- |
| Registry | `CBSORANPM664QXYMYRZKLQDQE2TXFSK4GBMC6EIRSRTAUZRZCRZRFNMK` | Immutable; namespace deployment salt version `1`; required ownership epoch |
| Allocator | `CCSORANYFHUJUWETEQ7UWIDN4JTPV7NN5G6YUSSBZI57X7EQWVA63VMJ` | Stored `SchemaVersion = 4` |
| Nova Registrar | `CBSORANHQRVZ7PEZ7VVFEQ7GQJ3M4RLBY2DNQ44MIVLU5C4J2DJ3DH3N` | `claim_version() = 1`; versioned `ClaimKey` entries below |
| Nova Resolver | `CDSORANQGIHRR7ZLQTARI6UJUA6TPVFUYO4BXKFJQPUXUVWLCJPGWA5H` | `payment_version() = 2`; `destination_version() = 2`; `initialization_version() = 1` |
| Primary | `CASORAN755O3GCQTRAHKDXLLCSDLNKAQWAP6MWABRSFVSHLJOEKAC7AB` | Immutable; no stored schema-version key |
| Lookup | `CDSORANQAJK35UV2HR63CMB6M5NYISHMUBTB6EQY2CZ3Y7HJDIOHRJWA` | `version() = 2`; `destination_version() = 2`; `muxed_identity_version() = 1` |
...
<Note>
This additive schema is active on testnet after the Lookup upgrade at ledger **4,549,326**. Verify the current Wasm hash and `muxed_identity_version() == 1` against [release status](/reference/release-status). The original deployment receipts remain unchanged.
</Note>
...
A newly uploaded replacement Wasm can have a shorter TTL than an existing Lookup instance. Because `keep_alive()` extends both, a later consumer write can inherit substantial code-extension cost if deployment upkeep has not been funded. Verify the code entry and instance separately after an upgrade, and confirm representative write fees before enabling consumers. Raising a consumer fee limit does not fix that deployment-maintenance gap.
...
The API maintenance keeper checks **contract instance and current Wasm-code TTLs independently** for Lookup, Registry, Allocator, Primary and known namespace Registrar/Resolver contracts. A long-lived instance does not establish that replacement code has the same remaining lifetime. See [release status](/reference/release-status#hosted-services) for deployment verification; source changes alone do not establish that a keeper is running.

Primary has no `keep_alive` method. Its permissionless `touch_primary(keeper_address)` can maintain code and instance even when that address has no Primary record. This call does not create an election or change a display name. Prepared maintenance transactions must preserve the exact requested invocation and contain no contract authorization entries. A namespace contract that asks for additional account authority is not signed by the keeper.

Record maintenance covers known Registry nodes, names, Resolver payment/profile/reverse records, Primary records and retained Allocator claims. Complete M elections use the exact `MuxedReverse(namespace, G, u64)` and `MuxedPrimary(G, u64)` keys. Current election events supply candidates, and each pass can recover up to 500 matching events from the indexer's stored event archive. This bounded history recovery is not enumeration of all on-chain keys. A touch checks an existing key; it does not recreate a missing election or make its forward proof valid.

The testnet infrastructure configuration caps a keeper transaction at **200 XLM** and cumulative reserved fees at **1,000 XLM per pass**. These are maintenance spending ceilings, not namespace claim prices, username prices or wallet transaction limits. The keeper reserves each simulated fee before signing, counts failed attempts against its attempt limit and stops further attempts after an uncertain submission. Unused allowance is not spent. Other networks require explicit maintenance fee limits.

Archived code, instances and records require restoration; a touch is not a restore. An absent optional record may never have existed or may have been intentionally cleared. Inspect the relevant contract state and event history before attempting restoration; absence alone does not justify recreating a record.

Retained Rejected/Withdrawn claims are maintained alongside active claims. The keeper also retains each known `(label, claimant)` pair after the current claim changes and independently checks its `ParkedAge`, `RejectedLock` and `ClaimantReopenEta` entries. It extends due, live entries through Stellar's ledger-entry TTL operation. `touch_claim` alone does not cover those separate keys.

Historical candidates come from exact fee and claimant-appeal events plus known claims. Events outside the indexer's coverage require verified historical evidence; a label-only event does not establish who the earlier claimant was. Unavailable entries require inspection before any restoration. Restoring an archived entry preserves its stored value and does not clear a rejection or grant a reopening.

## Read raw keys with Stellar RPC

This read-only Node example uses `@stellar/stellar-sdk@17.0.1` and `@sorandomains/lookup@0.8.0`. It reads a Registry namespace record and the Resolver's raw address, payment text and generation marker. It does not sign, submit, restore, or construct a payment. Replace the example name with an actually issued name.
