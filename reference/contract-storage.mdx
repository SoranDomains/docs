...
This Node example uses `@stellar/stellar-sdk@17.0.1` and `@sorandomains/lookup@0.10.1`. It reads a Registry namespace record and the Resolver's raw address, payment text and generation marker. It only reads state. Use an active name from the [live examples](/live-examples), or one you have issued.
...
| Registrar `live_records(nodes)` | Array of `null` or `[holderAddress, generationBigInt, builtinAddress]`; at most 16 nodes |
| Resolver `identity_read_version()` | `1` |
| Resolver `identity_context()` | `[registryAddress, registrarAddress, 1]` |
| Resolver `reverse_records(addresses)` | Array of `null` or `[nameString, nodeBytes, generationBigInt]`; at most 16 addresses |
| Resolver `identity_destinations(requests)` | Array of `null` or `[paymentDestination, explicitForwardBoolean]`; at most 16 requests |
| Primary `identity_read_version()` | `1` |
| Primary `primary_records(addresses)` | Array of `null` or `[nameString, namespaceNodeBytes]`; at most 8 addresses |
