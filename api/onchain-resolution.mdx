...
// Enforce the canonical grammar the contracts require — a non-canonical label
// silently hashes to a different (wrong) node, so validate rather than trust input.
const label = (s) => {
  const l = s.toLowerCase();
  if (!/^[a-z0-9-]{1,63}$/.test(l)) throw new Error(`invalid label: ${s}`);
  return Buffer.from(l, "utf8");
};
const nsNode  = (s) => sha(Buffer.concat([Buffer.alloc(32), sha(label(s))]));
const subNode = (parent, s) => sha(Buffer.concat([parent, sha(label(s))]));
...
// latest ledger's close time once and compare `expires_at` to it.
const now = BigInt((await server.getLatestLedger()).closeTime);
const liveAt = (name) => name && (name.expires_at === 0n || name.expires_at > now);
...
- **Archival.** `getLedgerEntries` reads only **live** ledger state, so an
  **empty** result (`entries: []`) is ambiguous: the key may never have been
  ...
  (`liveUntilLedgerSeq < latestLedger`). Either way a read-only indexer cannot
  and need not restore (restore is an owner/keeper write) — treat an
  archived/absent entry as stale/unavailable and keep your last-known value.
...
for (;;) {
  ...
  for (const ev of page.events) handle(ev);           // topic0 = the event name
  cursor = page.cursor;                                // persist it
  if (!page.events.length && page.latestLedger <= /* head you polled */ 0) break;
}
// filters: [{ type: "contract", contractIds: [REGISTRY, ...registrars] }]
```
...
- **Contract-ID cap.** A `getEvents` call accepts ~**25** contract IDs. Every
  namespace has its own Registrar and Resolver, so past ~12 namespaces you must
  shard the IDs across calls, one cursor per shard.
