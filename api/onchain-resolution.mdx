...
wrong address for any name with a divergent forward record. `addr` and
`primary_of` apply the on-chain **generation** gate; the built-in `resolve`
fallback applies the **expiry** gate. A raw entry read applies neither.
</Warning>
...
// A DataKey enum → ScVal. Every resolution key is a tuple variant:
//   DataKey::Name(node) → scvVec([scvSymbol("Name"), scvBytes(node)])
// (a unit variant would be a bare scvSymbol, but the only one — Config — is
//  instance storage, so it isn't read through this path.)
...
async function readEntry(contractId, keyScVal) {
  ...
  const e = entries[0];
  if (e.liveUntilLedgerSeq && e.liveUntilLedgerSeq < latestLedger) return null; // archived — needs restore
...
// name = { holder, address, expires_at: 0n, generation: 0n }
```

<Warning>
Raw reads bypass the on-chain gates, so to match the resolvers' actual answer
you must replicate them yourself:
- **Resolver precedence + generation.** Every Resolver record — forward
  `Addr(node)` → `AddrRec{ addr, generation }`, `Text(node, key)` →
  `TextRec{ value, generation }`, `Reverse(addr)` → `RevRec{ node, name, generation }`
  — carries the `generation` it was written under. A record only counts when
  its `generation` equals the Registrar `Name(node).generation`; a transfer or
  reissue bumps `Name.generation` and orphans the old records. Read both and
  compare; if they differ (or no `Addr` record exists), fall back to
  `Name.address`.
...
- **Archival.** An **empty** result (`entries: []`) means the key was never
  ...
  still be archived (rent lapsed): compare its `liveUntilLedgerSeq` to the
  response's `latestLedger`, and if it's in the past the value is not live and
  the entry needs restoring (the `readEntry` above already rejects it). Never
