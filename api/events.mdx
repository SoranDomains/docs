...
| Resolver | `pay_v1`, name node, generation | Complete direct `Payment { address, memo }` |
| Resolver | `pay_v2`, name node, generation | `PaymentDestination::Muxed(MuxedPayment { account, id })` |
| Registrar | `claimcfg` | Complete `ClaimConfig` |
| Registrar | `reserved`, name node | `(label:Bytes, reserved:bool, policy_version:u64)` |
| Registrar | `claim_v1`, namespace node, holder | `(request_id:BytesN<32>, ClaimReceipt)` |
...
`text_v1` is **invalidation only**. It does not contain the historic value. Re-read current text through Lookup if you need it; historic reconstruction needs historic state access. Direct G/C payment writes emit `pay_v1`; muxed writes emit `pay_v2`. Neither duplicates address/text companion events. The muxed event preserves the exact u64 ID separately from its G account. A configured memo-free `set_addr` emits the updated complete payment; an ordinary unconfigured address-only write emits `addr_v1`.
...
Native claim receipts are historical operation results; current holder and payment instructions still require current contract reads. Receipt replay emits no new claim event. Configuration and reservation changes are distinct from issuance. See [native stored types and event semantics](/reference/successor-claim-storage#ttl-restoration-and-events).
...
Hosted webhooks have their own [supported event list](/platform/webhooks). The addition of raw `pay_v1`/`pay_v2`, fee, text and renewal events does not automatically add those events to webhook subscriptions.

## Muxed display-name elections

The deployed muxed identity extension emits `set_mrev` / `clr_mrev` for namespace elections and `set_mpri` / `clr_mpri` for cross-namespace elections from **Universal Lookup**. The payloads retain the base G and exact u64 ID; a set event also contains the versioned name/routing/generation snapshot. See the [exact event and storage shapes](/reference/contract-storage#muxed-identity-storage) and [release status](/reference/release-status) before indexing them.

An event does not prove that an election is still valid. Recheck it through `reverse_muxed` or `primary_name_muxed`. Keep M IDs separate from G/C reverse keys and from customer memo values.
