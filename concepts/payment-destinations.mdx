...
These are payment-destination capabilities, not a promise that every wallet can sign for every account type. This page describes the deployed Resolver payment ABI 2, Lookup ABI 2, Lookup SDK 0.8.0 and Holder SDK 0.6.0 in [release status](/reference/release-status).

<Note>
The SDK examples target Lookup **0.8.0** and Holder **0.6.0**. Check [release status](/reference/release-status) for their publication status. M-address support was introduced in Lookup 0.6.0 and Holder 0.4.0 and is preserved by this release. The same instructions are available through the [contract methods](/api/onchain-resolution), [HTTP payment endpoint](/api/resolution) and app.
</Note>
...
G/C reverse and Primary names identify that account without distinguishing customer memos. The deployed [muxed identity extension](/sdk/reverse-and-primary) gives each complete M address its own elected reverse and Primary name, with no base-G or other-ID fallback. Use Lookup 0.8.0 and Holder 0.6.0 with `muxed_identity_version() == 1`; older packages do not provide M elections. See [release status](/reference/release-status).

The base G account must authorize an M election. For an exchange-controlled account, the exchange signs; having a customer M address does not give the customer its signing key. Changing a name to an M destination can invalidate an earlier G/C election. A M ID does not become a separate holder, namespace owner, or signing account.
