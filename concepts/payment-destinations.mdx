...
<Note>
The contracts, HTTP API and app support these destinations now. Lookup 0.6.0 and Holder 0.4.0 npm publication is pending, so the Soran SDK examples below require those releases once available. Use the live [contract methods](/api/onchain-resolution) or [HTTP payment endpoint](/api/resolution) for current integrations; older SDKs cannot preserve M destinations.
</Note>
