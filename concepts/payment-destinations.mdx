...
These are payment-destination capabilities, not a promise that every wallet can sign for every account type. This page describes the deployed Resolver payment ABI 2, Lookup ABI 2, Lookup SDK 0.7.0 and Holder SDK 0.5.0 in [release status](/reference/release-status).

<Note>
The SDK examples target Lookup **0.7.0** and Holder **0.5.0**. Check [release status](/reference/release-status) for their publication status. M-address support was introduced in Lookup 0.6.0 and Holder 0.4.0 and is preserved by this release. The same instructions are available through the [contract methods](/api/onchain-resolution), [HTTP payment endpoint](/api/resolution) and app.
</Note>
