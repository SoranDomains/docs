...
These are payment-destination capabilities, not a promise that every wallet can sign for every account type. This page describes the deployed Resolver and Lookup ABI 1, Lookup SDK 0.5.0 and Holder SDK 0.3.0 in [release status](/reference/release-status).
...
| Classic `G...` with an ID memo | Yes | Include the exact unsigned 64-bit ID as a transaction memo |
| Classic `G...` with a text memo | Yes | Include the exact 1–28 UTF-8 bytes as a transaction memo |
| Classic `G...` with a hash memo | Yes | Include the exact 32 bytes as a transaction hash memo |
